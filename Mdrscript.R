...
mdrtb_pdst1 <- mdrtb_pdst |> ageCats() |> select(-Province, -District, -Facility) |>
  mutate(Patient_Type = fct_recode(Patient_Type, 
                                   ...
                                   "Positive" = "Positive", 
                                   "Sputum smear negative relapse" = "SP_SM_NR",
                                   "Sputum smear positive relapse" = "SP_SM_PR" ),
...
geo_df <- mdrtb_pdst_gdf[mdrtb_pdst_gdf$MDR == "MDR", ] |> ageCats() |> 
  mutate(MOH_TB_TYPE_PATIENT = fct_recode(MOH_TB_TYPE_PATIENT, 
                                          "Initial Treatment Failure" = "FA_1STL_TR",
                                          "Failure After Retreatment" = "FA_RE_TREA",
                                          "MDR Contaminated" = "MDR_CONTA",
                                          "MDR Follow-UP" = "MDR_F",
                                          "New Patient" = "NEW",
                                          "Retreatment after initial treatment interruption" = "RE_AF_DEFA",
                                          "Positive" = "POSITIVE"),
         MOH_HIV_STATUS = fct_recode(MOH_HIV_STATUS, "Not Done" = "NOT_DONE",
                                     "Declined" = "DECLINED",
                                     "Negative" ="NEGATIVE",
